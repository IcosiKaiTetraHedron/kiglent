...
from kiglent.input.base import Device, AbsoluteAxis, RelativeAxis, Button
from kiglent.input.base import Joystick, Controller, AppleRemote, ControllerManager
from kiglent.input.controller import get_mapping, create_guid

from kiglent.libs.darwin.cocoapy import CFSTR, CFIndex, CFTypeID, known_cftypes
from kiglent.libs.darwin.cocoapy import kCFRunLoopDefaultMode, CFAllocatorRef, cf
from kiglent.libs.darwin.cocoapy import cfset_to_set, cftype_to_value, cfarray_to_list
...
iokit = kiglent.lib.load_library(framework='IOKit')
...
class KiglentDevice(Device):
    """
    Represents a device in the Kiglent input system.
    ...
    def __init__(self, display=None):
        ...
        def on_connect(hiddevice):
            if _controller := _create_controller(hiddevice, display):
                ...
                kiglent.app.platform_event_loop.post_event(self, 'on_connect', _controller)
        ...
        def on_disconnect(hiddevice):
            if hiddevice in self._controllers:
                ...
                kiglent.app.platform_event_loop.post_event(self, 'on_disconnect', _controller)
...
def get_devices(display=None):
    return [KiglentDevice(display, device) for device in _hid_manager.devices]


def get_joysticks(display=None):
    return [Joystick(KiglentDevice(display, device)) for device in _hid_manager.devices
...
def get_apple_remote(display=None):
    for device in _hid_manager.devices:
        if device.product == 'Apple IR':
            return AppleRemote(KiglentDevice(display, device))
...
def _create_controller(device, display):
    ...
    if device.is_joystick() or device.is_gamepad() or device.is_multi_axis():
        ...
        if mapping := get_mapping(device.get_guid()):
            return Controller(KiglentDevice(display, device, axis_filter=[0x1, 0x2]), mapping)
        else:
