...
from kiglent.input.win32.directinput import _di_manager as _di_device_manager
from kiglent.input.win32.directinput import DirectInputDevice, _create_controller

from kiglent.input.win32.directinput import get_devices as dinput_get_devices
from kiglent.input.win32.directinput import get_controllers as dinput_get_controllers
from kiglent.input.win32.directinput import get_joysticks

try:
    from kiglent.input.win32.wintab import get_tablets
except:
...
if not kiglent.options["win32_disable_xinput"]:
    try:
        from kiglent.input.win32.xinput import XInputControllerManager, XInputController, XInputDevice
        from kiglent.input.win32.xinput import _device_manager as _xinput_device_manager
        from kiglent.input.win32.xinput import get_devices as xinput_get_devices
        from kiglent.input.win32.xinput import get_controllers as xinput_get_controllers
        ...
        def on_connect(di_device):
            if di_device not in self._di_controllers:
                if self._add_di_controller(di_device):
                    kiglent.app.platform_event_loop.post_event(self, 'on_connect', self._di_controllers[di_device])
