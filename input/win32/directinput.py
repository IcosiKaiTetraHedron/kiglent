...
from kiglent.libs.win32.constants import WM_DEVICECHANGE, DBT_DEVICEARRIVAL, DBT_DEVICEREMOVECOMPLETE, \
    DBT_DEVTYP_DEVICEINTERFACE, DEVICE_NOTIFY_WINDOW_HANDLE

from kiglent.event import EventDispatcher

import kiglent
from kiglent.input import base
from kiglent.libs import win32
from kiglent.libs.win32 import dinput, _user32, DEV_BROADCAST_DEVICEINTERFACE, com, DEV_BROADCAST_HDR
from kiglent.libs.win32 import _kernel32
from kiglent.input.controller import get_mapping
from kiglent.input.base import ControllerManager
...
class DirectInputDevice(base.Device):
    ...
    def open(self, window=None, exclusive=False):
        ...
        if window is None:
            ...
            window = kiglent.gl._shadow_window
            for window in kiglent.app.windows:
                break
        ...
        kiglent.app.platform_event_loop.add_wait_object(self._wait_object, self._dispatch_events)
    ...
    def close(self):
        ...
        kiglent.app.platform_event_loop.remove_wait_object(self._wait_object)
...
class DIDeviceManager(EventDispatcher):
    ...
    def _get_devices(self, display=None):
        ...
        if not kiglent.options["win32_disable_xinput"]:
            try:
                from kiglent.input.win32.xinput import get_xinput_guids
    ...
    def _event_devicechange(self, msg, wParam, lParam):
        ...
        if wParam == DBT_DEVICEARRIVAL or wParam == DBT_DEVICEREMOVECOMPLETE:
            ...
            if hdr_ptr.contents.dbch_devicetype == DBT_DEVTYP_DEVICEINTERFACE:
                ...
                kiglent.app.platform_event_loop.post_event(self, '_recheck_devices')
        ...
        def on_connect(di_device):
            if di_device not in self._controllers:
                if self._add_controller(di_device):
                    kiglent.app.platform_event_loop.post_event(self, 'on_connect', self._controllers[di_device])
