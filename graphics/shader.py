...
import kiglent
from kiglent.gl import Context, GLException, gl, gl_info
...
from kiglent.graphics.vertexbuffer import AttributeBufferObject, BufferObject
...
_debug_gl_shaders = kiglent.options['debug_gl_shaders']
...
class UniformBlock:
    ...
    def set_binding(self, binding: int) -> None:
        ...
        Binding value of 0 is reserved for the Kiglent's internal uniform block named ``WindowBlock``.
        ...
        """
        assert binding != 0, "Binding 0 is reserved for the internal Kiglent 'WindowBlock'."
        assert kiglent.gl.current_context is not None, "No context available."
        manager: _UBOBindingManager = kiglent.gl.current_context.ubo_manager
...
def _introspect_uniform_blocks(program: ShaderProgram | ComputeShaderProgram) -> dict[str, UniformBlock]:
    ...
    for index in range(_get_number(program_id, gl.GL_ACTIVE_UNIFORM_BLOCKS)):
        ...
        if not hasattr(kiglent.gl.current_context, "ubo_manager"):
            kiglent.gl.current_context.ubo_manager = _UBOBindingManager()

        manager = kiglent.gl.current_context.ubo_manager
        ...
        if kiglent.options.shader_bind_management:
...
class ShaderProgram:
    ...
    def uniform_blocks(self) -> dict[str, UniformBlock]:
        ...
        :py:class:`~kiglent.graphics.shader.UniformBlock` instances.
    ...
    def _vertex_list_create(self, count: int, mode: int, indices: Sequence[int] | None = None,
        ...
        batch = batch or kiglent.graphics.get_default_batch()
        group = group or kiglent.graphics.ShaderGroup(program=self)
