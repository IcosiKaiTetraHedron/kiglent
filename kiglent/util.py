"""Various utility functions used internally by kiglent
"""
...
if sys.version_info >= (3, 12):
    ...
    Buffer = bytes | bytearray | memoryview | ctypes.Array
...
def debug_print(kiglent_option_name: str = 'debug') -> Callable[[str], bool]:
    """Get a debug printer controlled by the given ``kiglent.options`` name.
    ...
    #. Running Python with the ``-O`` flag makes kiglent run faster by
    ...
    Args:
        `kiglent_option_name` :
            The name of a key in :attr:`kiglent.options` to read the
    ...
    """
    enabled = kiglent.options.get(kiglent_option_name, False)
    if enabled:
...
class CodecRegistry:
    ...
    def encode(self, media, filename, file=None, **kwargs):
        """Attempt to encode a kiglent object to a specified format. All registered
...
class Decoder:
    ...
    def decode(self, *args, **kwargs):
        ...
        kiglent object. Throws DecodeException if there is an error.
