...
_debug_lib = kiglent.options['debug_lib']
_debug_trace = kiglent.options['debug_trace']

_is_kiglent_doc_run = getattr(sys, "is_kiglent_doc_run", False)

if kiglent.options['search_local_libs']:
    script_path = kiglent.resource.get_script_home()
    cwd = os.getcwd()
...
class LibraryLoader:  # noqa: D101
    ...
    platform = kiglent.compat_platform
    # this is only for library loading, don't include it in kiglent.platform
    if platform == 'cygwin':
        platform = 'win32'
