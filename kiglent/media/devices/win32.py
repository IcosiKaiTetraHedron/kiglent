...
from kiglent.libs.win32 import PROPVARIANT, com
from kiglent.libs.win32 import _ole32 as ole32
from kiglent.libs.win32.constants import CLSCTX_INPROC_SERVER
from kiglent.media.devices import base
...
class AudioNotificationCB(com.COMObject):
    ...
    def OnDeviceStateChanged(self, pwstrDeviceId, dwNewState):
        ...
        kiglent_old_state = Win32AudioDevice.platform_state[old_state]
        kiglent_new_state = Win32AudioDevice.platform_state[dwNewState]
        assert _debug(
            f"Audio device '{device.name}' changed state. From: {kiglent_old_state} to: {kiglent_new_state}")

        device.state = dwNewState
        self.audio_devices.dispatch_event('on_device_state_changed', device, kiglent_old_state, kiglent_new_state)
    ...
    def OnDefaultDeviceChanged(self, flow, role, pwstrDeviceId):
        ...
        if role == 0:
            ...
            kiglent_flow = Win32AudioDevice.platform_flow[flow]
            assert _debug(f"Default device was changed to: {device} ({kiglent_flow})")

            self.audio_devices.dispatch_event('on_default_changed', device, kiglent_flow)
