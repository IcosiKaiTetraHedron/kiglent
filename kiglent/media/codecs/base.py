...
from kiglent.media.exceptions import MediaException, CannotSeekException
from kiglent.util import next_or_equal_power_of_two

if TYPE_CHECKING:
    from kiglent.image import AbstractImage
    from kiglent.image.animation import Animation
    from kiglent.media.codecs import MediaEncoder
    from kiglent.media.drivers.base import MediaEvent
    from kiglent.media.player import Player
...
class AudioFormat:
    ...
    Args:
        channels (int): The number of channels: 1 for mono or 2 for stereo
            (kiglent does not yet support surround-sound sources).
...
class AudioData:
    ...
    This class is used internally by kiglent.
    ...
    Args:
        ...
        events (List[:class:`kiglent.media.drivers.base.MediaEvent`]): List of events
...
class Source:
    ...
    Attributes:
        is_player_source (bool): Determine if this source is a player
            ...
            Check on a :py:class:`~kiglent.media.player.Player` if this source
    ...
    def play(self) -> 'Player':
        ...
        Returns:
            :class:`.Player`
        """
        from kiglent.media.player import Player  # XXX Nasty circular dependency
        player = Player()
        player.queue(self)
        player.play()
    ...
    def get_animation(self) -> 'Animation':
        ...
        Returns:
            :class:`kiglent.image.Animation`
        """
        from kiglent.image import Animation, AnimationFrame
    ...
    def get_next_video_frame(self) -> Optional['AbstractImage']:
        ...
        Returns:
            :class:`kiglent.image.AbstractImage`: The next video frame image,
    ...
    def save(self,
        ...
        if encoder:
            ...
            import kiglent.media.codecs
            return kiglent.media.codecs.registry.encode(self, filename, file)
    ...
    def is_precise(self) -> bool:
        ...
        kiglent's internals are guaranteed to never make unaligned
        requests, or requests of less than 1024 bytes.

        If this method returns ``False``, kiglent will wrap the source in an
...
class StreamingSource(Source):
    ...
    :class:`~kiglent.media.player.Player`.
    """
...
class StaticSource(Source):
    ...
    Construct a :py:class:`~kiglent.media.StaticSource` for the data in
    ``source``.
    ...
    def get_audio_data(self, num_bytes: float, compensation_time: float = 0.0) -> Optional[AudioData]:
        ...
        :class:`~kiglent.media.player.Player`, it creates a
...
class StaticMemorySource(StaticSource):
    ...
    Do not use directly. This class is used internally by kiglent.
