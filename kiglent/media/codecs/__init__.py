...
from kiglent.util import CodecRegistry, Decoder, Encoder
from .base import *
...
def add_default_codecs():
    ...
    # preference.  This is called automatically by kiglent.media.
    ...
    if kiglent.compat_platform.startswith('linux'):
        try:
    ...
    try:
        if kiglent.compat_platform in ('win32', 'cygwin'):
            from kiglent.libs.win32.constants import WINDOWS_VISTA_OR_GREATER
    ...
    if kiglent.compat_platform.startswith("darwin"):
        try:
...
def have_ffmpeg():
    ...
    try:
        ...
        if not found and not _is_kiglent_doc_run:
