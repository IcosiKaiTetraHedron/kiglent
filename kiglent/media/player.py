...
from kiglent.gl import GL_TEXTURE_2D
from kiglent.media import buffered_logger as bl
from kiglent.media.drivers import get_audio_driver
from kiglent.media.codecs.base import Source, SourceGroup
...
class Player(kiglent.event.EventDispatcher):
    """High-level sound and video player."""
    ...
    def _set_playing(self, playing: bool) -> None:
        ...
        if playing and source:
            ...
            if source.video_format is not None:
                kiglent.clock.schedule_once(self.update_texture, 0)
            ...
            if self._audio_player is None and source.video_format is None:
                kiglent.clock.schedule_once(lambda dt: self.dispatch_event("on_eos"), source.duration)

        else:
            if self._audio_player is not None:
                self._audio_player.stop()

            kiglent.clock.unschedule(self.update_texture)
            self._timer.pause()
    ...
    def _create_texture(self) -> None:
        ...
        self._texture = kiglent.image.Texture.create(video_format.width, video_format.height, GL_TEXTURE_2D)
    ...
    def update_texture(self, dt: float | None = None) -> None:
        ...
        if ts is None:
            ...
            kiglent.clock.schedule_once(self._video_finished, 0)
            return
        elif ts > time:
            # update_texture called too early (probably manually!)
            kiglent.clock.schedule_once(self.update_texture, ts - time)
            return
        ...
        kiglent.clock.schedule_once(self.update_texture, delay)
        # self.pr.enable()
