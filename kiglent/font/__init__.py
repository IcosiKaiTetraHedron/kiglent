...
kiglent will automatically load any system-installed fonts.  You can add additional fonts
...
:meth:`add_directory`. These fonts are then available in the same way as system-installed fonts::
    ...
    # or
    from kiglent import resource
    resource.add_font('action_man.ttf')
    action_man = font.load('Action Man')

See the :mod:`kiglent.font.base` module for documentation on the base classes used
by this package.
"""
...
from kiglent.font.user import UserDefinedFontBase
...
def _get_system_font_class() -> type[Font]:
    ...
    Kiglent relies on OS dependent font systems for loading fonts and glyph creation.
    """
    if kiglent.compat_platform == "darwin":
        from kiglent.font.quartz import QuartzFont
        _font_class = QuartzFont

    elif kiglent.compat_platform in ("win32", "cygwin"):
        from kiglent.libs.win32.constants import WINDOWS_7_OR_GREATER
        if WINDOWS_7_OR_GREATER and not kiglent.options["win32_gdi_font"]:
            from kiglent.font.dwrite import Win32DirectWriteFont
            _font_class = Win32DirectWriteFont
        else:
            from kiglent.font.win32 import GDIPlusFont
            _font_class = GDIPlusFont
    else:
        from kiglent.font.freetype import FreeTypeFont
...
def add_user_font(font: UserDefinedFontBase) -> None:
    ...
    otherwise kiglent may not find the font later.
    ...
    Raises:
        Exception: If font provided is not derived from :py:class:`~kiglent.font.user.UserDefinedFontBase`.
    """
    ...
    if not hasattr(shared_object_space, "kiglent_font_font_cache"):
        shared_object_space.kiglent_font_font_cache = weakref.WeakValueDictionary()
        shared_object_space.kiglent_font_font_hold = []
        # Match a tuple to specific name to reduce lookups.
        shared_object_space.kiglent_font_font_name_match = {}
    font_cache = shared_object_space.kiglent_font_font_cache
    font_hold = shared_object_space.kiglent_font_font_hold
...
def load(name: str | Iterable[str] | None = None, size: float | None = None, weight: str = "normal",
    ...
    font_cache = shared_object_space.kiglent_font_font_cache
    font_hold = shared_object_space.kiglent_font_font_hold
    font_name_match = shared_object_space.kiglent_font_font_name_match
...
def add_file(font: str | BinaryIO | bytes) -> None:
    ...
    you should pass the face name (not the file name) to :meth::py:func:`kiglent.font.load` or any
...
def add_directory(directory: str) -> None:
    """Add a directory of fonts to kiglent's search path.

    This function simply calls :meth:`kiglent.font.add_file` for each file with a ``.ttf``
