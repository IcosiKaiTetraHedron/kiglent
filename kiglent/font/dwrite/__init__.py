...
import kiglent
from kiglent.font import base
from kiglent.font.base import Glyph, GlyphPosition
from kiglent.font.dwrite.d2d1_lib import (
...
from kiglent.font.dwrite.d2d1_types_lib import D2D1_COLOR_F, D2D_POINT_2F
from kiglent.font.dwrite.dwrite_lib import (
...
from kiglent.font.harfbuzz import get_harfbuzz_shaped_glyphs, get_resource_from_dw_font, harfbuzz_available
from kiglent.image.codecs.wincodec_lib import GUID_WICPixelFormat32bppPBGRA
from kiglent.libs.win32 import UINT16, UINT32, UINT64, com
from kiglent.libs.win32 import _kernel32 as kernel32
from kiglent.libs.win32.constants import (
    LOCALE_NAME_MAX_LENGTH,
    WINDOWS_8_1_OR_GREATER,
    WINDOWS_10_1809_OR_GREATER,
    WINDOWS_10_CREATORS_UPDATE_OR_GREATER,
)
from kiglent.util import debug_print

if TYPE_CHECKING:
    from kiglent.image import ImageData

_debug_font = kiglent.options["debug_font"]
_debug_print = debug_print("debug_font")


try:
    from kiglent.image.codecs import wic
...
class DirectWriteGlyphRenderer(base.GlyphRenderer):  # noqa: D101
    ...
    antialias_mode = D2D1_TEXT_ANTIALIAS_MODE_DEFAULT if kiglent.options.text_antialiasing is True else D2D1_TEXT_ANTIALIAS_MODE_ALIASED
    ...
    def render_to_image(self, text: str, width: int, height: int) -> ImageData:
        """This process takes Kiglent out of the equation and uses only DirectWrite to shape and render text.
...
class Win32DirectWriteFont(base.Font):
    ...
    texture_internalformat = kiglent.gl.GL_RGBA
    ...
    def __init__(self, name: str, size: float, weight: str = "normal", italic: bool | str = False,  # noqa: D107
        ...
        if self.locale is None:
            self.locale = ""
            self.rtl = False  # Right to left should be handled by kiglent?
        ...
        if kiglent.options["dw_legacy_naming"] and (self._font_index is None and self._collection is None):
        ...
        if kiglent.options.text_shaping == 'harfbuzz' and harfbuzz_available():
    ...
    def get_glyphs(self, text: str) -> tuple[list[Glyph], list[base.GlyphPosition]]:
        ...
        if kiglent.options.text_shaping == 'platform':
