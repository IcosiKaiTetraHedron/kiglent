...
from kiglent.graphics import shader, vertexdomain
from kiglent.graphics.vertexarray import VertexArray  # noqa: F401
from kiglent.graphics.vertexbuffer import BufferObject

if TYPE_CHECKING:
    from kiglent.graphics.groups import Group
    from kiglent.graphics.shader import ShaderProgram
    from kiglent.graphics.vertexdomain import IndexedVertexList, VertexList

_debug_graphics_batch = kiglent.options['debug_graphics_batch']
...
def get_default_batch() -> Batch:
    ...
    try:
        return kiglent.gl.current_context.kiglent_graphics_default_batch
    except AttributeError:
        kiglent.gl.current_context.kiglent_graphics_default_batch = Batch()
        return kiglent.gl.current_context.kiglent_graphics_default_batch
...
def get_default_shader() -> ShaderProgram:
    ...
    try:
        return kiglent.gl.current_context.object_space.kiglent_graphics_default_shader
    except AttributeError:
        ...
        kiglent.gl.current_context.object_space.kiglent_graphics_default_shader = _shader_program
        return kiglent.gl.current_context.object_space.kiglent_graphics_default_shader

def get_default_blit_shader() -> ShaderProgram:
    """A default basic shader for blitting, provides no blending."""
    try:
        return kiglent.gl.current_context.object_space.kiglent_graphics_default_blit_shader
    except AttributeError:
        _vertex_shader = shader.Shader(_blit_vertex_source, 'vertex')
        _fragment_shader = shader.Shader(_blit_fragment_source, 'fragment')
        _shader_program = shader.ShaderProgram(_vertex_shader, _fragment_shader)
        kiglent.gl.current_context.object_space.kiglent_graphics_default_blit_shader = _shader_program
        return kiglent.gl.current_context.object_space.kiglent_graphics_default_blit_shader
...
class Batch:
    ...
    Many drawable kiglent objects accept an optional `Batch` argument in their
    constructors. By giving a `Batch` to multiple objects, you can tell kiglent
    ...
    then draws the entire batch::
        ...
        car = kiglent.sprite.Sprite(car_image, batch=batch)
        boat = kiglent.sprite.Sprite(boat_image, batch=batch)
    ...
    def _update_draw_list(self) -> None:
        ...
        self.top_groups.sort(reverse=True)

        draw_list = []
        stack = [(group, False) for group in self.top_groups if group.visible]

        while stack:
            group, unset = stack.pop()
            if unset:
                ...
                if domain_map:
                    for (indexed, instanced, mode, formats), domain in domain_map.items():
                        if domain.is_empty:
                ...
                if children:
                    ...
                    stack.extend((child, False) for child in children if child.visible)
    ...
    def draw_subset(self, vertex_lists: Sequence[VertexList | IndexedVertexList]) -> None:
        ...
        stack = [group for group in self.top_groups if group.visible]
        ...
        while stack:
            ...
            if unset:
                ...
                if children:
                    children.sort(reverse = True)
                    stack.extend(child for child in children if child.visible)


from kiglent.graphics.groups import *
