...
draw event handler::
    ...
    ball_image = kiglent.image.load('ball.png')
    ball = kiglent.sprite.Sprite(ball_image, x=50, y=50)

    window = kiglent.window.Window()
...
The sprite can be moved by modifying the :py:attr:`~kiglent.sprite.Sprite.x` and
:py:attr:`~kiglent.sprite.Sprite.y` properties.  Other
properties determine the sprite's :py:attr:`~kiglent.sprite.Sprite.rotation`,
:py:attr:`~kiglent.sprite.Sprite.scale` and
:py:attr:`~kiglent.sprite.Sprite.opacity`.
...
the sprite (:since: kiglent 1.2).

The sprite's positioning, rotation and scaling all honor the original
image's anchor (:py:attr:`~kiglent.image.AbstractImage.anchor_x`,
:py:attr:`~kiglent.image.AbstractImage.anchor_y`).
...
creates one hundred ball sprites and adds each of them to a :py:class:`~kiglent.graphics.Batch`.  The
entire batch of sprites is then drawn in one call::
    ...
    for i in range(100):
        x, y = i * 10, 50
        ball_sprites.append(kiglent.sprite.Sprite(ball_image, x, y, batch=batch))
...
:py:mod:`kiglent.graphics` for more details on batched rendering, and grouping of
...
import kiglent
from kiglent import clock, event, graphics, image
...
if TYPE_CHECKING:
    ...
    from kiglent.image import AbstractImage, Animation, Texture
...
def get_default_shader() -> ShaderProgram:
    ...
    """
    return kiglent.gl.current_context.create_program((vertex_source, 'vertex'),
...
class SpriteGroup(graphics.Group):
    ...
    def __init__(self, texture: Texture, blend_src: int, blend_dest: int,
        ...
        The group is created internally when a :py:class:`~kiglent.sprite.Sprite`
