...
from kiglent.util import CodecRegistry, Decoder, Encoder, DecodeException, EncodeException
...
class ImageDecoder(Decoder):
    ...
    def decode_animation(self, filename, file):
        """Decode the given file object and return an instance of :py:class:`~kiglent.image.Animation`.
        Throws ImageDecodeException if there is an error.  filename
        can be a file type hint.
        """
...
def add_default_codecs():
    ...
    # preference.  This is called automatically by kiglent.image.

    # Compressed texture in DDS format
    try:
        from kiglent.image.codecs import dds
    ...
    if compat_platform == 'darwin':
        try:
            from kiglent.image.codecs import quartz
    ...
    if compat_platform in ('win32', 'cygwin'):
        try:
            from kiglent.image.codecs import gdiplus
    ...
    if compat_platform.startswith('linux'):
        try:
            from kiglent.image.codecs import gdkpixbuf2
    ...
    # Fallback: PIL
    try:
        from kiglent.image.codecs import pil
    ...
    try:
        from kiglent.image.codecs import png
    ...
    try:
        from kiglent.image.codecs import bmp
