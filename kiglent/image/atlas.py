...
:py:class:`~kiglent.image.atlas.TextureAtlas` maintains one texture; :py:class:`TextureBin`
manages a collection of atlases of a given size. :py:class:`TextureArrayBin` works similarly
except for :py:class:`~kiglent.image.TextureArray`s instead of altases.

This module is used internally by the :py:mod:`kiglent.resource` module.

Example usage::

    # Load images from disk
    car_image = kiglent.image.load('car.png')
    boat_image = kiglent.image.load('boat.png')
...
if TYPE_CHECKING:
    from kiglent.image import AbstractImage, ImageData, TextureRegion, TextureArrayRegion
...
class TextureAtlas:
    ...
    When creating a TextureAtlas instance, a new :py:class:`~kiglent.image.Texture`
    ...
    def __init__(self, width: int = 2048, height: int = 2048) -> None:
        ...
        self.texture = kiglent.image.Texture.create(width, height)
        self.allocator = Allocator(width, height)

    def add(self, img: ImageData, border: int = 0) -> TextureRegion:
        """Add ImageData to the atlas.

        Given :py:class:`~kiglent.image.ImageData`, add it to the Atlas and
        return a new :py:class:`~kiglent.image.TextureRegion` object. An
...
class TextureBin:
    ...
    :py:class:`~kiglent.image.atlas.TextureBin` maintains a collection
...
class TextureArrayBin:
    ...
    :py:class:`~kiglent.image.atlas.TextureArrayBin` maintains a collection of
    ...
    def __init__(self, texture_width: int = 2048, texture_height: int = 2048, max_depth: int | None = None) -> None:
        max_texture_size = kiglent.image.get_max_texture_size()
        self.max_depth = max_depth or kiglent.image.get_max_array_texture_layers()
        self.texture_width = min(texture_width, max_texture_size)
        self.texture_height = min(texture_height, max_texture_size)
        self.arrays = []
    ...
    def add(self, img: ImageData) -> TextureArrayRegion:
        ...
        This method calls :py:meth:`~kiglent.image.TextureArray.add` for the first
        ...
        except kiglent.image.TextureArrayDepthExceeded:
            pass
        except IndexError:
            pass

        array = kiglent.image.TextureArray.create(self.texture_width, self.texture_height, max_depth=self.max_depth)
