...
from kiglent import app, display, gl, window

if TYPE_CHECKING:
    from kiglent.gl import Config
...
class Display:
    ...
    def __init__(self, name: str = None, x_screen: int = None) -> None:
        ...
        screen number to use with this display.  A kiglent display can only be 
        ...
        screen to be accessed through a single kiglent`~kiglent.display.Display`
...
class Screen(abc.ABC):
    ...
    def get_best_config(self, template: Config = None) -> Config:
        ...
        :class:`~kiglent.window.NoSuchConfigException` will be raised.
        A configuration supported by the platform that best fulfils
        the needs described by the template.

        :deprecated: Use :meth:`kiglent.gl.Config.match`.
...
class Canvas:
    ...
    Canvases are used internally by kiglent to represent drawing areas --
