...
(and so are available by importing ``kiglent.gl``):

``kiglent.gl.gl``
    OpenGL
``kiglent.gl.gl.glext_arb``
    ARB registered OpenGL extension functions
``kiglent.gl.gl.gl_compat``
...
import kiglent as _kiglent
from kiglent import compat_platform
from kiglent.gl.gl import *  # Must always be imported before gl_info or bad things happen.  # noqa: F403
from kiglent.gl import gl_info  # noqa: F401

from kiglent.gl.gl_compat import GL_INTENSITY, GL_LUMINANCE
from kiglent.gl.lib import GLException  # noqa: F401
...
_is_kiglent_doc_run = hasattr(_sys, "is_kiglent_doc_run") and _sys.is_kiglent_doc_run
...
if _kiglent.options['debug_texture']:
...
def _create_shadow_window() -> None:
    ...
    import kiglent
    if not kiglent.options['shadow_window'] or _is_kiglent_doc_run:
        return
...
elif _kiglent.options['headless']:
...
# Import kiglent.window now if it isn't currently being imported (this creates the shadow window).
if not _is_kiglent_doc_run and 'kiglent.window' not in _sys.modules and _kiglent.options['shadow_window']:
    # trickery is for circular import
    _kiglent.gl = _sys.modules[__name__]
    import kiglent.window  # noqa: F401
