...
from kiglent.libs.win32 import constants
from kiglent.libs.win32.types import (
    BITMAPINFOHEADER,
    BYTE,
    COLORREF,
    HCURSOR,
...
from kiglent.libs.win32.winkey import chmap, keymap
...
import kiglent
from kiglent.display.win32 import Win32Canvas
from kiglent.event import EventDispatcher
from kiglent.libs.win32 import (
    BITMAPV5HEADER,
    DWM_BLURBEHIND,
    MAKEINTRESOURCE,
    RAWINPUTDEVICE,
...
class Win32Window(BaseWindow):
    ...
    def set_icon(self, *images: kiglent.image.ImageData) -> None:
        ...
        def best_image(width: int, height: int) -> kiglent.image.ImageData:
        ...
        def get_icon(img: kiglent.image.ImageData) -> HICON:
        ...
        if inp.header.dwType == constants.RIM_TYPEMOUSE:
            ...
            if rmouse.usFlags & 0x01 == constants.MOUSE_MOVE_RELATIVE:
                if rmouse.lLastX != 0 or rmouse.lLastY != 0:
                    ...
                    # We invert it for Kiglent so positive is motion up.
    ...
    def _event_dpi_change(self, msg: int, wParam: int, lParam: int) -> int:
        ...
        if not self._fullscreen and \
                (kiglent.options.dpi_scaling != "real" or constants.WINDOWS_10_CREATORS_UPDATE_OR_GREATER):
