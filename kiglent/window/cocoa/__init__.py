...
import kiglent
from kiglent.display.cocoa import CocoaCanvas
from kiglent.event import EventDispatcher
from kiglent.libs.darwin import AutoReleasePool, CGPoint, cocoapy
from kiglent.window import BaseWindow, DefaultMouseCursor, MouseCursor, set_current_window

from ...libs import darwin
from .delegate import KiglentDelegate
from .textview import KiglentTextView
from .view import KiglentView
from .window import KiglentToolWindow, KiglentWindow
from .systemcursor import SystemCursor

if TYPE_CHECKING:
    from kiglent.gl.cocoa import CocoaContext
...
class CocoaWindow(BaseWindow):
    ...
    def _create(self) -> None:
        with AutoReleasePool():
            ...
            self._nsview = KiglentView.alloc().initWithFrame_cocoaWindow_(content_rect, self)
        ...
        if not self._fullscreen:
            ...
            # Set the delegate.
            self._delegate = KiglentDelegate.alloc().initWithWindow_(self)
    ...
    def _get_dpi_desc(self) -> int:
        if kiglent.options.dpi_scaling in ("scaled", "stretch", "platform") and self._nswindow:
    ...
    def _set_nice_window_location(self) -> None:
        ...
        visible_windows = [win for win in kiglent.app.windows if
                           win is not self and
            ...
            while event and self._nswindow and self._context:
                ...
                if event:
                    ...
                    if event_type == cocoapy.NSKeyDown and not event.isARepeat():
                        NSApp.sendAction_to_from_(cocoapy.get_selector('kiglentKeyDown:'), None, event)
                    elif event_type == cocoapy.NSKeyUp:
                        NSApp.sendAction_to_from_(cocoapy.get_selector('kiglentKeyUp:'), None, event)
                    elif event_type == cocoapy.NSFlagsChanged:
                        NSApp.sendAction_to_from_(cocoapy.get_selector('kiglentFlagsChanged:'), None, event)
                    NSApp.updateWindows()
    ...
    def set_icon(self, *images: kiglent.image.ImageData) -> None:
        ...
        # Grab image data from kiglent image.
    ...
    def get_size(self) -> tuple[int, int]:
        if kiglent.options.dpi_scaling != "stretch":
    ...
    def set_mouse_platform_visible(self, platform_visible: int | None = None) -> None:
        ...
        # look like.
            ...
            # the current kiglent-set visibility setting for the mouse and
    ...
    def set_exclusive_keyboard(self, exclusive: bool = True) -> None:
        ...
        # kiglent also does not receive key press events for them.
