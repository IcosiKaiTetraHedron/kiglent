...
from kiglent.libs.darwin.cocoapy import (
    NSRectEncoding,
    NSUInteger,
    NSUIntegerEncoding,
    ObjCClass,
...
if TYPE_CHECKING:
    from kiglent.libs.darwin import ObjCInstance


class KiglentWindow_Implementation:
    KiglentWindow = ObjCSubclass('NSWindow', 'KiglentWindow')

    @KiglentWindow.method('B')
    ...
    @KiglentWindow.method(b'@' + NSUIntegerEncoding + b'@@B')
    ...
    @KiglentWindow.method(b'd' + NSRectEncoding)
    def animationResizeTime_(self, newFrame: ObjCInstance) -> float:
        return 0.0


class KiglentToolWindow_Implementation:
    KiglentToolWindow = ObjCSubclass('NSPanel', 'KiglentToolWindow')

    @KiglentToolWindow.method(b'@' + NSUIntegerEncoding + b'@@B')
    def nextEventMatchingMask_untilDate_inMode_dequeue_(self, mask: ObjCInstance, date: ObjCInstance,
                                                        mode: ObjCInstance, dequeue: bool) -> int:
        if self.inLiveResize():
            # Call the idle() method while we're stuck in a live resize event.
    ...
    # Need this for set_size to not flash.
    @KiglentToolWindow.method(b'd' + NSRectEncoding)
    def animationResizeTime_(self, newFrame: ObjCInstance) -> float:
        return 0.0


KiglentWindow = ObjCClass('KiglentWindow')
KiglentToolWindow = ObjCClass('KiglentToolWindow')
