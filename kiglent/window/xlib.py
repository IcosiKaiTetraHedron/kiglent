...
import kiglent
from kiglent.display.xlib import XlibCanvas, XlibScreenXinerama
from kiglent.event import EventDispatcher
from kiglent.libs.x11 import cursorfont, xlib
from kiglent.util import asbytes
from kiglent.window import (
    BaseWindow,
    DefaultMouseCursor,
    ImageMouseCursor,
    MouseCursor,
...
if TYPE_CHECKING:
    ...
    from kiglent.gl.xlib import XlibDisplayConfig

try:
    from kiglent.libs.x11 import xsync

    _have_xsync = True
except ImportError:
    _have_xsync = False

_debug = kiglent.options['debug_x11']
...
class XlibException(WindowException):
    ...
    This exception is probably a programming error in kiglent."""
...
class XlibWindow(BaseWindow):
    ...
    def _create(self) -> None:
        ...
        if not self._window:
            ...
            self._scale = self._screen.get_scale() if kiglent.options.dpi_scaling == "stretch" else 1.0
            ...
            if self._fullscreen:
                ...
                if kiglent.options.dpi_scaling in ("scaled", "stretch"):
    ...
    def _event_symbol(self, event):
        # kiglent.self.key keysymbols are identical to X11 keysymbols, no
    ...
    def _event_text_symbol(self, ev: xlib.XEvent) -> tuple[str | None, int]:
        ...
        # kiglent.self.key keysymbols are identical to X11 keysymbols, no
        # need to map the keysymbol.  For keysyms outside the kiglent set, map
    ...
    def _translate_button(value: int) -> int:
        ...
        contents defined in :py:module:`~kiglent.window.mouse`. This
