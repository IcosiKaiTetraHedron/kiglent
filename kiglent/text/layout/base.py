...
import kiglent
from kiglent import graphics
from kiglent.font.base import GlyphPosition
from kiglent.gl import (
    GL_BLEND,
    GL_DEPTH_ATTACHMENT,
    GL_DEPTH_COMPONENT,
    GL_LINES,
...
from kiglent.graphics import Group
from kiglent.text import runlist

if TYPE_CHECKING:
    from kiglent.customtypes import AnchorX, AnchorY, ContentVAlign, HorizontalAlign
    from kiglent.font.base import Font, Glyph
    from kiglent.graphics import Batch
    from kiglent.graphics.shader import ShaderProgram
    from kiglent.graphics.vertexdomain import VertexList
    from kiglent.image import Texture
    from kiglent.text.document import AbstractDocument, InlineElement
    from kiglent.text.runlist import AbstractRunIterator, RunIterator
...
def get_default_layout_shader() -> ShaderProgram:
    ...
    return kiglent.gl.current_context.create_program((layout_vertex_source, "vertex"),
...
def get_default_decoration_shader() -> ShaderProgram:
    ...
    return kiglent.gl.current_context.create_program((decoration_vertex_source, "vertex"),
...
class TextLayoutGroup(graphics.Group):
    ...
    The group is created internally when a :py:class:`~kiglent.text.Label`
...
class TextLayout:
    ...
    :py:func:`~kiglent.text.Label` and :py:func:`~kiglent.text.HTMLLabel` provide a convenient interface to this class.
    ...
    def document(self) -> AbstractDocument:
        ...
        For :py:class:`~kiglent.text.layout.IncrementalTextLayout` it is
    ...
    def x(self) -> float:
        ...
        See also :py:attr:`~kiglent.text.layout.TextLayout.anchor_x`.
        """
        return self._x
    ...
    def y(self) -> float:
        ...
        See also :py:attr:`~kiglent.text.layout.TextLayout.anchor_y`.
        """
        return self._y
    ...
    def rotation(self) -> float:
        ...
        See :py:attr:`~kiglent.text.layout.TextLayout.anchor_x`, and :py:attr:`~kiglent.text.layout.TextLayout.anchor_y`.
        """
    ...
    def position(self) -> tuple[float, float, float]:
        ...
        See also :py:attr:`~kiglent.text.layout.TextLayout.anchor_x`,
        and :py:attr:`~kiglent.text.layout.TextLayout.anchor_y`.
        """
    ...
    def content_width(self) -> int:
        ...
        user defined :py:attr:`~kiglent.text.layout.TextLayout.width`.
    ...
    def content_height(self) -> int:
        ...
        user defined :py:attr:`~kiglent.text.layout.TextLayout.height`.
        """
    ...
    def height(self) -> int | None:
        ...
        text when :py:attr:`~kiglent.text.layout.TextLayout.anchor_y` and
        :py:attr:`~kiglent.text.layout.TextLayout.content_valign` are
        used.
        """
        return self._height
    ...
    def anchor_y(self) -> AnchorY:
        ...
        See also :py:attr:`~kiglent.text.layout.TextLayout.content_valign`.
        """
    ...
    def get_as_texture(self, min_filter: int=GL_NEAREST, mag_filter: int=GL_NEAREST) -> Texture:
        """Utilizes a :py:class:`~kiglent.image.framebuffer.Framebuffer` to draw the current layout into a texture.
        ...
        .. versionadded:: 2.0.11
        """
        framebuffer = kiglent.image.Framebuffer()
        temp_pos = self.position
        width = int(round(self._content_width))
        height = int(round(self._content_height))
        texture = kiglent.image.Texture.create(width, height, min_filter=min_filter, mag_filter=mag_filter)
        depth_buffer = kiglent.image.buffer.Renderbuffer(width, height, GL_DEPTH_COMPONENT)
