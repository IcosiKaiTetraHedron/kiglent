...
if TYPE_CHECKING:
    ...
    from kiglent.image import AbstractImage, Texture
    from kiglent.resource import Location
    from kiglent.text.document import InlineElement
    from kiglent.text.layout import TextLayout


class _InlineElementGroup(kiglent.graphics.Group):
...
class ImageElement(kiglent.text.document.InlineElement):
    ...
    def place(self, layout: TextLayout, x: float, y: float, z: float, line_x: float, line_y: float, rotation: float,
        ...
        program = kiglent.text.layout.get_default_image_layout_shader()
        ...
        vertex_list = program.vertex_list_indexed(4, kiglent.gl.GL_TRIANGLES, [0, 1, 2, 0, 2, 3],
                                                  layout.batch, group,
...
class StructuredTextDecoder(kiglent.text.DocumentDecoder):  # noqa: D101
    def decode(self, text: str, location: Location | None=None) -> kiglent.text.document.FormattedDocument:
        self.len_text = 0
        self.current_style = {}
        self.next_style = {}
        self.stack = []
        self.list_stack = []
        self.document = kiglent.text.document.FormattedDocument()
        if location is None:
            location = kiglent.resource.FileLocation("")
