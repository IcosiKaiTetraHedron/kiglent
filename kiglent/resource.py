...
if TYPE_CHECKING:
    ...
    from kiglent.graphics.shader import Shader
    from kiglent.image import AbstractImage, Texture, TextureRegion
    from kiglent.image.animation import Animation
    from kiglent.image.atlas import TextureBin
    from kiglent.media.codecs import Source
    from kiglent.model import Scene
    from kiglent.text.document import AbstractDocument
...
def get_script_home() -> str:
    ...
    full path to the real script can be specified in :py:attr:`kiglent.resource.path`.
    """
...
def get_settings_path(name: str) -> str:
    ...
    """
    if kiglent.compat_platform in ('cygwin', 'win32'):
    ...
    elif kiglent.compat_platform.startswith('linux'):
...
class Loader:
    ...
    def add_font(self, filename: str) -> None:
        ...
        Fonts not installed on the system must be added to kiglent before they
    ...
    def _alloc_image(self, name: str, use_atlas: bool, border: int) -> AbstractImage:
        ...
        try:
            img = kiglent.image.load(name, file=fileobj)
        finally:
            fileobj.close()
    ...
    def _get_texture_atlas_bin(self, width: int, height: int, border: int) -> TextureBin | None:
        ...
        max_texture_size = kiglent.image.get_max_texture_size()
        ...
        try:
            ...
            texture_bin = kiglent.image.atlas.TextureBin()
    ...
    def image(self, name: str, flip_x: bool = False, flip_y: bool = False, rotate: Literal[0, 90, 180, 270, 360] = 0,
        ...
        packed into a :py:class:`~kiglent.image.atlas.TextureBin` (TextureAtlas)
        ...
        Args:
            ...
            atlas:
                ...
                kiglent. If atlas loading is not appropriate for specific texturing
    ...
    def animation(self, name: str, flip_x: bool = False, flip_y: bool = False,
        ...
        try:
            ...
            _animation = kiglent.image.load_animation(name, self.file(name))
    ...
    def media(self, name: str, streaming: bool = True) -> Source:
        ...
        The meaning of ``streaming`` is as for :py:func:`~kiglent.media.load`.
    ...
    def texture(self, name: str) -> Texture:
        ...
        fileobj = self.file(name)
        textureobj = kiglent.image.load(name, file=fileobj).get_texture()
    ...
    def scene(self, name: str) -> Scene:
        ...
        return kiglent.model.load(filename=abspathname, file=self.file(name))

    def html(self, name: str) -> AbstractDocument:
        """Load an HTML document."""
        self._ensure_index()
        fileobj = self.file(name)
        return kiglent.text.load(name, fileobj, 'text/html')

    def attributed(self, name: str) -> AbstractDocument:
        """Load an attributed text document.

        See `kiglent.text.formats.attributed` for details on this format.
        """
        self._ensure_index()
        fileobj = self.file(name)
        return kiglent.text.load(name, fileobj, 'text/vnd.kiglent-attributed')

    def text(self, name: str) -> AbstractDocument:
        """Load a plain text document."""
        self._ensure_index()
        fileobj = self.file(name)
        return kiglent.text.load(name, fileobj, 'text/plain')
    ...
    def shader(self, name: str, shader_type: str | None = None) -> Shader:
        ...
        return kiglent.graphics.shader.Shader(source_string, shader_type)
