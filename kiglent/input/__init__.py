...
:py:class:`~kiglent.window.Window`.  At the lowest
...
At this level kiglent does not try to interpret *what* a particular device is,
...
and the Apple remote control. These devices can usually be identified by kiglent
...
if TYPE_CHECKING:
    from kiglent.display import Display
...
if _is_kiglent_doc_run:
    ...
    def get_tablets(display: Display | None = None) -> list[Tablet]:
        ...
        kiglent does not currently support multiple tablets, and the behaviour
