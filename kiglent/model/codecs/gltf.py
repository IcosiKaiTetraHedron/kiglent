...
from kiglent.gl import GL_BYTE, GL_UNSIGNED_BYTE, GL_SHORT, GL_UNSIGNED_SHORT, GL_FLOAT, GL_DOUBLE
from kiglent.gl import GL_INT, GL_UNSIGNED_INT, GL_ELEMENT_ARRAY_BUFFER, GL_ARRAY_BUFFER
from kiglent.gl import GL_REPEAT
...
class Buffer:
    ...
    def __init__(self, data):
        ...
        if self._uri.startswith('data'):
            ...
            self._file = kiglent.resource.file(self._uri, 'rb')
...
def load_gltf(filename, file=None) -> GLTF:
    ...
    try:
        if file is None:
            file = kiglent.resource.file(filename, 'r')
        elif file.mode != 'r':
            file.close()
            file = kiglent.resource.file(filename, 'r')
    except kiglent.resource.ResourceNotFoundException:
