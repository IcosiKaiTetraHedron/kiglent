...
Styled text in kiglent is represented by one of the :py:class:`~kiglent.text.document.AbstractDocument` classes,
...
``"italic"``, it stores only the style names.  The kiglent layout classes give
...
:py:class:`~kiglent.text.document.InlineElement`.  An inline element behaves
...
Any class implementing :py:class:`~kiglent.text.document.AbstractDocument` provides an interface to a
document model as described above.  In theory a structured document such as
HTML or XML could export this model, though the classes provided by kiglent
implement only unstructured documents.

The :py:class:`~kiglent.text.document.UnformattedDocument` class assumes any styles set are set over the entire
...
The :py:class:`~kiglent.text.document.FormattedDocument` class implements the document model directly, using
the `RunList` class to represent style runs efficiently.

Style attributes
================

The following character style attribute names are recognised by kiglent:

``font_name``
    Font family name, as given to :py:func:`kiglent.font.load`.
``font_size``
    Font size, in points.
``weight``
    String.
...
The following paragraph style attribute names are recognised by kiglent.  Note
...
from kiglent import event
from kiglent.text import runlist

if TYPE_CHECKING:
    from kiglent.font.base import Font
    from kiglent.text.layout import TextLayout
...
class InlineElement(ABC):
    ...
    The kiglent layout classes reserve space in the layout for elements and
...
class AbstractDocument(event.EventDispatcher):
    """Abstract document interface used by all :py:mod:`kiglent.text` classes.

    This class can be overridden to interface kiglent with a third-party
    document format.  It may be easier to implement the document format in
    terms of one of the supplied concrete classes :py:class:`~kiglent.text.document.FormattedDocument` or
    :py:class:`~kiglent.text.document.UnformattedDocument`.
    """
    ...
    def text(self) -> str:
        ...
        For efficient incremental updates, use the :py:func:`~kiglent.text.document.AbstractDocument.insert_text` and
        :py:func:`~kiglent.text.document.AbstractDocument.delete_text` methods instead of replacing this property.
    ...
    def get_font_runs(self, dpi: int | None = None) -> runlist.AbstractRunIterator:
        """Get a style iterator over the `kiglent.font.Font` instances used in the document.
    ...
    def insert_text(self, start: int, text: str, attributes: dict[str, Any] | None = None) -> None:  # noqa: D417
        ...
        Dispatches an :py:meth:`~kiglent.text.document.AbstractDocument.on_insert_text` event.
    ...
    def insert_element(self, position: int, element: InlineElement,
        ...
        See the :py:class:`~kiglent.text.document.InlineElement` class documentation for details of usage.
    ...
    def set_style(self, start: int, end: int, attributes: dict[str, Any]) -> None:  # noqa: D417
        ...
        Dispatches an :py:meth:`~kiglent.text.document.AbstractDocument.on_style_text` event.
...
class FormattedDocument(AbstractDocument):
    ...
    :py:class:`~kiglent.text.document.AbstractDocument`.  All styles default to ``None``.
    """
