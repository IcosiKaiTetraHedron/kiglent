...
HTML files and a kiglent-specific markup format.  Documents can be styled with
...
A plain text label can be created with::
    ...
    label = kiglent.text.Label('Hello, world',
...
Alternatively, a styled text label using HTML can be created with::
    ...
    label = kiglent.text.HTMLLabel('<b>Hello</b>, <i>world</i>',
                                  x=10, y=10)
...
For details on the subset of HTML supported, see `kiglent.text.formats.html`.
...
from kiglent.text import caret, document, layout  # noqa: F401


if TYPE_CHECKING:
    from kiglent.customtypes import AnchorX, AnchorY, HorizontalAlign, Color4, ColorValue
    ...
    from kiglent.text.document import AbstractDocument, FormattedDocument, UnformattedDocument
...
SupportedMimeTypes = Literal["text/plain", "text/html", "text/vnd.kiglent-attributed"]
...
def get_decoder(filename: str | None, mimetype: SupportedMimeTypes | None = None) -> DocumentDecoder:
    ...
    ``text/html``
        HTML 4 Transitional
    ``text/vnd.kiglent-attributed``
        Attributed text; see `kiglent.text.formats.attributed`
    ...
    if mimetype == "text/plain":
        from kiglent.text.formats import plaintext
        return plaintext.PlainTextDecoder()
    if mimetype == "text/html":
        from kiglent.text.formats import html
        return html.HTMLDecoder()
    if mimetype == "text/vnd.kiglent-attributed":
        from kiglent.text.formats import attributed
...
def load(filename: str,
    ...
    location = kiglent.resource.FileLocation(_dirname(filename))
...
def decode_attributed(text: str) -> FormattedDocument:
    ...
    See `kiglent.text.formats.attributed` for a description of attributed text.
    """
    decoder = get_decoder(None, "text/vnd.kiglent-attributed")
...
class DocumentLabel(layout.TextLayout):
    ...
    @color.setter
    def color(self, color: ColorValue) -> None:
        r, g, b, *a = color
    ...
    def font_name(self) -> str | list[str]:
        ...
        The font name, as passed to :py:func:`kiglent.font.load`.  A list of names can
    ...
    def get_style(self, name: str) -> Any:
        ...
        `kiglent.text.document.STYLE_INDETERMINATE` is returned.

        Args:
            name:
                Style name to query.  See documentation from `kiglent.text.layout` for known style names.
        """
    ...
    def set_style(self, name: str, value: Any) -> None:
        ...
        Args:
            name:
                ...
                `kiglent.text.layout` for known style names.
            value:
                Value of the style.
        """
...
class HTMLLabel(DocumentLabel):
    ...
    A subset of HTML 4.01 is supported.  See `kiglent.text.formats.html` for
    details.
    """
