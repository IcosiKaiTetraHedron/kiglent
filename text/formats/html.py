...
list item with a tab, as the kiglent document model does not allow
...
import kiglent
from kiglent.text.formats import structured
...
class HTMLDecoder(HTMLParser, structured.StructuredTextDecoder):
    ...
    def get_image(self, filename: str) -> AbstractImage:
        return kiglent.image.load(filename, file=self.location.open(filename))
