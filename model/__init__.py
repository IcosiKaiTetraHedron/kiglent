...
The :py:mod:`~kiglent.model` module provides an interface for loading 3D "scenes"
and models. A :py:class:`~kiglent.model.Scene` is a logical container that can
...
only contains a single model (mesh)::
    ...
    scene = kiglent.model.load('teapot.obj')
...
You can also load scenes with :py:meth:`~kiglent.resource.scene`.
See :py:mod:`~kiglent.resource` for more information.
"""
...
import kiglent
from kiglent import gl, graphics
...
if TYPE_CHECKING:
    ...
    from kiglent.image import Texture
    from kiglent.model.codecs import ModelDecoder
...
def get_default_shader() -> ShaderProgram:
    return kiglent.gl.current_context.create_program((MaterialGroup.default_vert_src, 'vertex'),
                                                    (MaterialGroup.default_frag_src, 'fragment'))


def get_default_textured_shader() -> ShaderProgram:
    return kiglent.gl.current_context.create_program((TexturedMaterialGroup.default_vert_src, 'vertex'),
...
class Model:
    ...
    def __init__(self, vertex_lists: list[VertexList], groups: list[Group], batch: Batch | None = None) -> None:
        ...
        Args:
            vertex_lists:
                A list of :py:class:`~kiglent.graphics.VertexList` or
                :py:class:`~kiglent.graphics.IndexedVertexList`.
            groups:
                A list of :py:class:`~kiglent.model.TexturedMaterialGroup`, or
                 :py:class:`~kiglent.model.MaterialGroup`. Each group corresponds
...
class Cube(Model):
    ...
    def __init__(self, width=1.0, height=1.0, depth=1.0, color=(1.0, 1.0, 1.0, 1.0),
        ...
        self._group = kiglent.model.MaterialGroup(material=self._material, program=self._program, parent=group)
    ...
    def _create_vertexlist(self):
        ...
        return self._program.vertex_list_indexed(len(vertices) // 3, kiglent.gl.GL_TRIANGLES, indices,
