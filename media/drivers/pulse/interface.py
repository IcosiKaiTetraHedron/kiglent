...
from kiglent.media.drivers.pulse import lib_pulseaudio as pa
from kiglent.media.exceptions import MediaException
from kiglent.util import debug_print

if TYPE_CHECKING:
    from kiglent.media.codecs import AudioData, AudioFormat
...
class PulseAudioMainloop:
    ...
    def _get_app_name(self) -> str:
        ...
        # TODO move app name into kiglent.app (also useful for OS X menu bar?).
        return sys.argv[0]
...
class PulseAudioStream(PulseAudioMainloopChild):
    ...
    def create_sample_spec(self, audio_format: 'AudioFormat') -> pa.pa_sample_spec:
        """
        Create a PulseAudio sample spec from kiglent audio format.
        """
        _FORMATS = {
