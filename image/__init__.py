...
Once loaded, images can be used directly by most other modules of kiglent.  All
...
form).  To use a texture with kiglent.gl::
...
from kiglent.graphics.shader import Attribute
from kiglent.graphics.vertexbuffer import BufferObject
from kiglent.util import asbytes
...
class AbstractImage(ABC):
    ...
    def get_texture(self, rectangle: bool = False) -> Texture:
        """A :py:class:`~kiglent.image.Texture` view of this image.
    ...
    def get_mipmapped_texture(self) -> Texture:
        """Retrieve a :py:class:`~kiglent.image.Texture` instance with all mipmap levels filled in."""
...
class TextureSequence(AbstractImageSequence):
    ...
    Typical implementations store multiple :py:class:`~kiglent.image.TextureRegion`s
    within one :py:class:`~kiglent.image.Texture` to minimise state changes.
    """
...
class ImageData(AbstractImage):
    ...
    def get_mipmapped_texture(self) -> Texture:
        ...
        If :py:class:`~kiglent.image.set_mipmap_Image` has been called with at least
...
class Texture(AbstractImage):
    ...
    def __init__(self, width: int, height: int, target: int, tex_id: int,
        ...
        self._context = kiglent.gl.current_context
    ...
    def bind_image_texture(self, unit: int, level: int = 0, layered: bool = False,
        ...
        """Bind as an ImageTexture for use with a :py:class:`~kiglent.shader.ComputeShaderProgram`.
    ...
    def get_image_data(self, z: int = 0) -> ImageData:
        ...
        if kiglent.gl.current_context.get_info().get_opengl_api() == "gles":
            fbo = c_uint()
    ...
    def blit(self, x: int, y: int, z: int = 0, width: int | None = None, height: int | None = None) -> None:
        ...
        Instead, consider creating a :py:class:`~kiglent.sprite.Sprite` with the Texture,
        and drawing it as part of a larger :py:class:`~kiglent.graphics.Batch`.
        """
        x1 = x - self.anchor_x
        y1 = y - self.anchor_y
        ...
        program = kiglent.graphics.get_default_blit_shader()
        program.use()
    ...
    def get_transform(self, flip_x: bool = False, flip_y: bool = False,
        ...
        :py:meth:`~kiglent.image.AbstractImage.get_image_data` will return the
...
class TextureArray(Texture, UniformTextureSequence):
    ...
    def add(self, image: kiglent.image.ImageData) -> TextureArrayRegion:
...
class TileableTexture(Texture):
    ...
    Use :py:class:`~kiglent.image.create_for_image` classmethod to construct.
    """
    ...
    def blit_tiled(self, x: int, y: int, z: int, width: int, height: int) -> None:
        ...
        glActiveTexture(GL_TEXTURE0)
        glBindTexture(self.target, self.id)
        kiglent.graphics.draw_indexed(4, GL_TRIANGLES, [0, 1, 2, 0, 2, 3],
                                     position=('f', vertices),
...
class ImageGrid(AbstractImage, AbstractImageSequence):
    ...
    as a :py:class:`~kiglent.image.TextureGrid`::

        image_grid = ImageGrid(...)
        texture_grid = image_grid.get_texture_sequence()

    or as a :py:class:`~kiglent.image.Texture3D`::
...
class TextureGrid(TextureRegion, UniformTextureSequence):
    ...
    To construct, create an :py:class:`~kiglent.image.ImageGrid` first::

        image_grid = ImageGrid(...)
        texture_grid = TextureGrid(image_grid)

    The texture grid can be accessed as a single texture, or as a sequence
    of :py:class:`~kiglent.image.TextureRegion`.  When accessing as a sequence, you can specify
...
class BufferManager:
    ...
    Use :py:func:`~kiglent.image.get_buffer_manager` to obtain the instance
...
def get_buffer_manager() -> BufferManager:
    ...
    context = kiglent.gl.current_context
