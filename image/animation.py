...
Animations can be used by the :py:class:`~kiglent.sprite.Sprite` class in place
...
frame. You can load Animations from disk, such as from GIF files::
    ...
    ani = kiglent.resource.animation('walking.gif')
    sprite = kiglent.sprite.Sprite(img=ani)
...
by using the :py:meth:`~Animation.from_image_sequence` method::
    ...
    images = [kiglent.resource.image('walk_a.png'),
              kiglent.resource.image('walk_b.png'),
              kiglent.resource.image('walk_c.png')]

    ani = kiglent.image.Animation.from_image_sequence(images, duration=0.1, loop=True)

You can also use an :py:class:`kiglent.image.ImageGrid`, which is iterable::

    sprite_sheet = kiglent.resource.image('my_sprite_sheet.png')
    image_grid = kiglent.image.ImageGrid(sprite_sheet, rows=1, columns=5)

    ani = kiglent.image.Animation.from_image_sequence(image_grid, duration=0.1)
...
:py:class:`~AnimationFrame`::
    ...
    image_a = kiglent.resource.image('walk_a.png')
    image_b = kiglent.resource.image('walk_b.png')
    image_c = kiglent.resource.image('walk_c.png')

    frame_a = kiglent.image.AnimationFrame(image_a, duration=0.1)
    frame_b = kiglent.image.AnimationFrame(image_b, duration=0.2)
    frame_c = kiglent.image.AnimationFrame(image_c, duration=0.1)

    ani = kiglent.image.Animation(frames=[frame_a, frame_b, frame_c])
...
if TYPE_CHECKING:
    ...
    from kiglent.image import AbstractImage, AbstractImageSequence
...
class Animation:
    ...
    def add_to_texture_bin(self, texture_bin: TextureBin, border: int = 0) -> None:
        """Add the images of the animation to a :py:class:`~kiglent.image.atlas.TextureBin`.
