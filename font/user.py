"""This module defines the usage and creation of user defined fonts in Kiglent.

Previously, kiglent only supported font renderers that are built into the operating system, such as
...
The :py:class:`~kiglent.font.user.UserDefinedMappingFont` is provided for most use cases, which will allow you to
...
With :py:class:`~kiglent.font.user.UserDefinedMappingFont` you can pass a mapping of characters that point to your
:py:class:`~kiglent.image.ImageData`.
...
Once your font is created, you also must register it within kiglent to use it. This can be done through the
 :py:func:`~kiglent.font.add_user_font` function.
...
:py:func:`~kiglent.font.user.get_scaled_user_font`. By providing the user defined font instance, and a new size, you will
...
class UserDefinedGlyphRenderer(base.GlyphRenderer):
    ...
    def render(self, image_data: ImageData) -> Glyph:
        if self._font._scaling:  # noqa: SLF001
            ...
            new_image = kiglent.image.ImageData(image_resized.width, image_resized.height,
    ...
    def __init__(self, name: str, default_char: str, size: int, mappings: DictLikeObject,
        ...
        Args:
            ...
            mappings:
                ...
                The ``get`` function must take a string character, and output :py:class:`~kiglent.image.ImageData` if
