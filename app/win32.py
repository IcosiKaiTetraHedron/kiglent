...
from kiglent.libs.win32 import _kernel32, _user32, types, constants
from kiglent.libs.win32.types import HANDLE, TIMECAPS
...
class Win32EventLoop(PlatformEventLoop):
    def __init__(self):
        ...
        # that since event loop is created on kiglent.app import, whatever
        # imports kiglent.app _must_ own the main run loop.
        msg = types.MSG()
    ...
    def start(self):
        if _kernel32.GetCurrentThreadId() != self._event_thread:
            raise RuntimeError('EventLoop.run() must be called from the same ' +
                               'thread that imports kiglent.app')
