...
default policy is to wait until all windows are closed)::
    ...
    event_loop = kiglent.app.EventLoop()
...
from kiglent.app.base import EventLoop


_is_kiglent_doc_run = hasattr(sys, "is_kiglent_doc_run") and sys.is_kiglent_doc_run

if _is_kiglent_doc_run:
    from kiglent.app.base import PlatformEventLoop
else:
    if compat_platform == 'darwin':
        from kiglent.app.cocoa import CocoaPlatformEventLoop as PlatformEventLoop

        if platform.machine() == 'arm64' or kiglent.options["osx_alt_loop"]:
            from kiglent.app.cocoa import CocoaAlternateEventLoop as EventLoop
    elif compat_platform in ('win32', 'cygwin'):
        from kiglent.app.win32 import Win32EventLoop as PlatformEventLoop
    else:
        from kiglent.app.xlib import XlibEventLoop as PlatformEventLoop
...
:class:`kiglent.window.Window` are automatically added to this set upon 
...
def run(interval: float | None = 1 / 60) -> None:
    ...
    This is a convenience function, equivalent to::
        ...
        kiglent.app.event_loop.run(interval)
