...
from kiglent import app, clock, event

if TYPE_CHECKING:
    from kiglent.event import EventDispatcher
    from kiglent.window import BaseWindow
...
class EventLoop(event.EventDispatcher):
    ...
    operating system events, calls :py:func:`kiglent.clock.tick` to call
    scheduled functions and calls :py:meth:`kiglent.window.Window.on_draw` and
    :py:meth:`kiglent.window.Window.flip` to update window contents.
    ...
    def run(self, interval: float | None = 1/60) -> None:
        ...
        argument is not changed, schedules calling the :py:meth:`kiglent.window.Window.draw`
        ...
        Args:
            interval:
                ...
                If ``interval is None``, kiglent will not schedule calls to the
                :py:meth:`kiglent.window.Window.draw` method. Users must schedule
    ...
    def enter_blocking(self) -> None:
        """Called by kiglent internal processes when the operating system is about to block due to a user interaction.
    ...
    def exit_blocking() -> None:
        """Called by kiglent internal processes when the blocking operation completes.
    ...
    def idle(self) -> None | float:
        ...
        :py:meth:`kiglent.window.Window.on_draw` event for all windows and uses
        :py:func:`kiglent.clock.tick` and :py:func:`kiglent.clock.get_sleep_time`
        ...
        :py:func:`kiglent.clock.schedule` methods.
