"""kiglent is a cross-platform games and multimedia package based on kiglent.
see https://github.com/IcosiKaiTetraHedron/kiglent"""
...
@dataclass
class Options:
    """Dataclass for global kiglent options."""
    ...
    """If ``True``, will print more verbose information when :py:class:`~kiglent.font.base.Font`'s are loaded."""
    ...
    """If ``True``, all calls to OpenGL functions are checked afterwards for
     ...
     with the -O option).  It is disabled by default when kiglent is "frozen", such as
    ...
    """If ``True``, prints batch information being drawn, including :py:class:`~kiglent.graphics.Group`'s, VertexDomains,
    and :py:class:`~kiglent.image.Texture` information. This can be useful to see how many Group's are being
    consolidated."""
    ...
    """If ``True``, prints information on :py:class:`~kiglent.image.Texture` size (in bytes) when they are allocated and
    deleted."""
    ...
    """By default, kiglent creates a hidden window with a GL context when
     kiglent.gl is imported.  This allows resources to be loaded before
    ...
    """If set, the `kiglent.window.Window.vsync` property is ignored, and
     this option overrides it (to either force vsync on or off).  If unset,
     or set to None, the `kiglent.window.Window.vsync` property behaves
     as documented.
     """

    xsync: bool = True
    """If ``True`` (the default), kiglent will attempt to synchronise the
    ...
    """If ``False``, kiglent won't try to search for libraries in the script
    ...
    """If ``True``, kiglent will fallback to the legacy ``GDIPlus`` font renderer for Windows. This may provide
    ...
    is useful when running kiglent on a headless server, or compute cluster. OpenGL drivers with ``EGL`` support are
    required for this mode.
    """

    headless_device: int = 0
    """If using ``headless`` mode (``kiglent.options['headless'] = True``), this option allows you to set which
    ...
    com_mta: bool = False
    """If ``True``, this will enforce COM Multithreaded Apartment Mode for Windows applications. By default, kiglent
    ...
    Keep in mind that kiglent objects may not be scaled proportionately, so this is left up to the developer.
    The :py:attr:`~kiglent.window.Window.scale` & :py:attr:`~kiglent.window.Window.dpi` attributes can be queried as a
    ...
    by Kiglent. Final sizes are dictated by the platform the window was created on. It is up to the user to make any
    ...
    """If ``True``, this will enable internal management of Uniform Block bindings for
     :py:class:`~kiglent.graphics.shader.ShaderProgram`'s.

    If ``False``, bindings will not be managed by Kiglent. The user will be responsible for either setting the binding
...
#: Instance of :py:class:`~kiglent.Options` used to set runtime options.
options: Options = Options()


for _option_name, _type_str in options.__annotations__.items():
    """Check Environment Variables for kiglent options"""
...
class _ModuleProxy:
    ...
    def __getattr__(self, name: str): # noqa: ANN204
        try:
            ...
            import_name = f"kiglent.{self._module_name}"
...
if TYPE_CHECKING:
    ...
    matrix = _ModuleProxy("matrix")  # type: ignore
    ...
    vector = _ModuleProxy("vector")  # type: ignore
