...
Call `kiglent.app.run` to enter the main event loop (by default, this
returns when all open windows are closed)::

    from kiglent import app
    app.run()

Creating a game window
----------------------

Use :py:meth:`~kiglent.window.Window.set_exclusive_mouse` to hide the mouse
cursor and receive relative mouse movement events.  Specify ``fullscreen=True``
as a keyword argument to the :py:class:`~kiglent.window.Window` constructor to
...
import kiglent
from kiglent import gl
from kiglent.event import EVENT_HANDLE_STATE, EventDispatcher
from kiglent.graphics import shader
from kiglent.matrix import Mat4
from kiglent.window import key, mouse

if TYPE_CHECKING:
    from kiglent.display.base import Display, Screen, ScreenMode
    from kiglent.gl import DisplayConfig, Config, Context
    from kiglent.text import Label

_is_kiglent_doc_run = hasattr(sys, 'is_kiglent_doc_run') and sys.is_kiglent_doc_run
...
class MouseCursor:
    ...
    def draw(self, x: int, y: int) -> None:
        ...
        coordinates.  The projection is set to the kiglent default (i.e.,
...
class ImageMouseCursor(MouseCursor):
    ...
    def __init__(self, image: kiglent.image.AbstractImage, hot_x: int = 0, hot_y: int = 0,
...
class BaseWindow(EventDispatcher, metaclass=_WindowMetaclass):
    ...
    #:      :py:meth:`~kiglent.window.Window.on_close` handler when `kiglent.app.event_loop` is being
    #:      used.
    has_exit: bool = False

    #: Window display contents validity.  The :py:mod:`kiglent.app` event loop
    #: examines every window each iteration and only dispatches the :py:meth:`~kiglent.window.Window.on_draw`
    ...
    #: in the window's :py:meth:`~kiglent.window.Window.on_draw` handler, and setting it to True again in
    ...
    _projection_matrix: Mat4 = Mat4()
    _view_matrix: Mat4 = Mat4()
    ...
    def __init__(self,
        ...
        ``config`` as returned from :py:meth:`~kiglent.display.Screen.get_matching_configs` or similar.

        The context will be active as soon as the window is created, as if
        :py:meth:`~kiglent.window.Window.switch_to` was just called.
        ...
        self._style = style
        if kiglent.options['vsync'] is not None:
            self._vsync = kiglent.options['vsync']
        else:
            self._vsync = vsync
    ...
    def close(self) -> None:
        ...
        The :py:meth:`kiglent.app.EventLoop.on_window_close` event is
        dispatched by the :py:attr:`kiglent.app.event_loop` when this method
        is called.
        """
        from kiglent import app
    ...
    def dispatch_events(self) -> None:
        ...
        This method is provided for legacy applications targeting kiglent 1.0,
        and advanced applications that must integrate their event loop
        into another framework.

        Typical applications should use :py:func:`kiglent.app.run`.
        """

    def draw(self, dt: float) -> None:
        """Redraw the Window contents.

        This method will first call the :py:meth:`~kiglent.window.Window.`switch_to`
        method to make the GL context current. It then dispatches the
        :py:meth:`~kiglent.window.Window.on_draw` and
        :py:meth:`~kiglent.window.Window.on_refresh`
        events. Finally, it calls the :py:meth:`~kiglent.window.Window.flip`
    ...
    def flip(self) -> None:
        ...
        The default :py:attr:`~kiglent.app.event_loop` automatically
        calls this method after the window's
        :py:meth:`~kiglent.window.Window.on_draw` event.
        """
    ...
    def get_system_mouse_cursor(self, name: str) -> MouseCursor:
        ...
        Use :py:meth:`~kiglent.window.Window.set_mouse_cursor` to make the cursor returned by this method
    ...
    def set_fullscreen(self, fullscreen: bool = True, screen: Screen | None = None, mode: ScreenMode | None = None,
        ...
        Args:
            ...
            mode:
                ...
                have been obtained by enumerating :py:meth:`~kiglent.display.Screen.get_modes`.  If
    ...
    def set_mouse_platform_visible(self, platform_visible: bool | None = None) -> None:
        ...
        :see: :py:meth:`~kiglent.window.Window.set_mouse_visible` instead.
    ...
    def set_icon(self, *images: kiglent.image.AbstractImage) -> None:
        """Set the window icon.
    ...
    def switch_to(self) -> None:
        ...
        calling :py:meth:`~kiglent.gl.Context.set_current`. The latter
    ...
    def scale(self) -> float:
        ...
        Read only.
        """
        if kiglent.options.dpi_scaling != "real":
            return self._dpi / 96
    ...
    def view(self) -> Mat4:
        ...
        :py:class:`~kiglent.math.Mat4` instance can be set.
    ...
    if _is_kiglent_doc_run:
        ...
        def on_close(self) -> EVENT_HANDLE_STATE:
            ...
            The default handler sets `has_exit` to ``True``.  In kiglent 1.1, if
            `kiglent.app.event_loop` is being used, `close` is also called,
        ...
        def on_context_state_lost(self) -> EVENT_HANDLE_STATE:
            ...
            kiglent may sometimes need to recreate the window's GL context if
        ...
        def on_key_press(self, symbol: int, modifiers: int) -> EVENT_HANDLE_STATE:
            ...
            Since kiglent 1.1 the default handler dispatches the :py:meth:`~kiglent.window.Window.on_close`
        ...
        def on_move(self, x: int, y: int) -> EVENT_HANDLE_STATE:
            ...
            .. note: This is one of few methods in kiglent which use a Y-down coordinate system.
        ...
        def on_refresh(self, dt: float) -> EVENT_HANDLE_STATE:
            ...
            The ``EventLoop`` will dispatch this event when the ``draw``
            method has been called. The window will already have the
            GL context, so there is no need to call :py:meth:`~kiglent.window.Window.switch_to`. The window's
            :py:meth:`~kiglent.window.Window.flip` method will be called immediately after this event, so your
        ...
        def on_resize(self, width: int, height: int) -> EVENT_HANDLE_STATE:
            ...
            there is no need to call :py:meth:`~kiglent.window.Window.switch_to` in this handler.
        ...
        def on_text(self, text: str) -> EVENT_HANDLE_STATE:
            ...
            Typically, this is called after :py:meth:`~kiglent.window.Window.on_key_press` and before
            :py:meth:`~kiglent.window.Window.on_key_release`, but may also be called multiple times if the key
        ...
        def on_text_motion_select(self, motion: int) -> EVENT_HANDLE_STATE:
            ...
            events rather than the raw :py:meth:`~kiglent.window.Window.on_key_press`, as different platforms
            have different default keyboard mappings, and key repeats are
            handled correctly.

            The values that `motion` can take are defined in :py:mod:`kiglent.window.key`:
...
class FPSDisplay:
    ...
    at the end of the windows' :py:meth:`~kiglent.window.Window.on_draw` event handler::

        from kiglent.window import Window, FPSDisplay
    ...
    def __init__(self, window: kiglent.window.Window, color: tuple[int, int, int, int] = (127, 127, 127, 127),
...
current_window: BaseWindow | None = None
def get_current_window() -> BaseWindow | None:
    return current_window
def set_current_window(window: BaseWindow) -> None:
...
if _is_kiglent_doc_run:
    ...
    if kiglent.options['headless']:
        from kiglent.window.headless import HeadlessWindow as Window
    elif kiglent.compat_platform == 'darwin':
        from kiglent.window.cocoa import CocoaWindow as Window
    elif kiglent.compat_platform in ('win32', 'cygwin'):
        from kiglent.window.win32 import Win32Window as Window
    else:
        from kiglent.window.xlib import XlibWindow as Window

# Create shadow window. (trickery is for circular import)
if not _is_kiglent_doc_run:
    kiglent.window = sys.modules[__name__]
